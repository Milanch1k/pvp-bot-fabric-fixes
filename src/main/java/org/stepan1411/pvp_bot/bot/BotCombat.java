...
public class BotCombat {
    ...
    public static void update(ServerPlayerEntity bot, net.minecraft.server.MinecraftServer server) {
        ...
        switch (state.currentMode) {
            ...
            case CRYSTAL -> handleCrystalPvp(bot, target, state, distance, settings, server);
        }
    }
    ...
    private static void selectWeaponMode(ServerPlayerEntity bot, CombatState state, double distance, BotSettings settings) {
        ...
        boolean hasCrystalGear = hasObsidian(inventory) && hasEndCrystal(inventory);
        ...
        double crystalRange = 8.0; // Crystal PVP эффективен до 8 блоков
        
        // Логика выбора оружия
        if (hasCrystalGear && distance <= crystalRange && distance > 2.0 && settings.isCrystalPvpEnabled()) {
            // Crystal PVP - если есть обсидиан и кристаллы, враг на средней дистанции
    ...
    /**
     * Crystal PVP - обсидиан + кристалл + удар
     * Цикл:
     * 1. Ставим обсидиан РЯДОМ с врагом (как можно ближе)
     * 2. Ставим на обсидиан End Crystal
     * 3. Бьём кристалл (взрыв)
     * 4. Если враг убежал - повторяем цикл
     * 5. Если враг рядом - спамим шаги 2 и 3
     */
    private static void handleCrystalPvp(ServerPlayerEntity bot, Entity target, CombatState state, double distance, BotSettings settings, net.minecraft.server.MinecraftServer server) {
        var inventory = bot.getInventory();
        var world = bot.getEntityWorld();
        ...
        state.crystalPvpTicks++;
        
        // Смотрим на врага
        BotNavigation.lookAt(bot, target);
        
        // Проверяем дистанцию до врага
        boolean enemyNearby = distance <= 4.0; // Враг рядом
        boolean enemyFarAway = distance > 6.0; // Враг убежал
        
        // Если враг убежал - сбрасываем цикл и начинаем заново
        if (enemyFarAway) {
            state.crystalPvpStep = 0;
            state.lastObsidianPos = null;
            state.crystalPvpTicks = 0;
            // Идём к врагу
            BotNavigation.moveToward(bot, target, settings.getMoveSpeed());
            return;
        }
        ...
        switch (state.crystalPvpStep) {
            case 0: // Шаг 1: Ставим обсидиан рядом с врагом
                // Переключаемся на обсидиан только в первый тик этого шага
                if (state.crystalPvpTicks == 1) {
                    int obsidianSlot = findObsidian(inventory);
                    if (obsidianSlot < 0) {
                        // Нет обсидиана - переключаемся на ближний бой
                        state.currentMode = CombatState.WeaponMode.MELEE;
                        return;
                    }
                    
                    // Перемещаем обсидиан в хотбар если нужно
                    if (obsidianSlot >= 9) {
                        ItemStack obsidian = inventory.getStack(obsidianSlot);
                        ItemStack current = inventory.getStack(0);
                        inventory.setStack(obsidianSlot, current);
                        inventory.setStack(0, obsidian);
                        obsidianSlot = 0;
                    }
                    
                    // Переключаем на обсидиан
                    ((org.stepan1411.pvp_bot.mixin.PlayerInventoryAccessor) inventory).setSelectedSlot(obsidianSlot);
                }
                
                // Ставим обсидиан на 5-м тике
                if (state.crystalPvpTicks == 5) {
                    net.minecraft.util.math.BlockPos obsidianPos = findBestObsidianPosition(bot, target, world);
                    
                    if (obsidianPos != null) {
                        // Смотрим на блок
                        Vec3d blockCenter = new Vec3d(obsidianPos.getX() + 0.5, obsidianPos.getY() + 0.5, obsidianPos.getZ() + 0.5);
                        Vec3d botPos = bot.getEyePos();
                        double dx = blockCenter.x - botPos.x;
                        double dy = blockCenter.y - botPos.y;
                        double dz = blockCenter.z - botPos.z;
                        double horizontalDist = Math.sqrt(dx * dx + dz * dz);
                        float yaw = (float) (Math.atan2(dz, dx) * (180.0 / Math.PI)) - 90.0f;
                        float pitch = (float) -(Math.atan2(dy, horizontalDist) * (180.0 / Math.PI));
                        bot.setYaw(yaw);
                        ...
                        try {
                            ...
                            state.crystalPvpStep = 1; // Переходим к шагу 2
                            state.crystalPvpTicks = 0;
                        } catch (Exception e) {
                            // Не удалось поставить - пробуем снова
                            state.crystalPvpTicks = 0;
                        }
                    } else {
                        // Не нашли позицию - пробуем снова
                        state.crystalPvpTicks = 0;
                    }
                }
                break;
                
            case 1: // Шаг 2: Ставим кристалл на обсидиан
                // Переключаемся на кристалл только в первый тик этого шага
                if (state.crystalPvpTicks == 1) {
                    int crystalSlot = findEndCrystal(inventory);
                    if (crystalSlot < 0) {
                        // Нет кристаллов - переключаемся на ближний бой
                        state.currentMode = CombatState.WeaponMode.MELEE;
                        return;
                    }
                    
                    // Перемещаем кристалл в хотбар если нужно
                    if (crystalSlot >= 9) {
                        ItemStack crystal = inventory.getStack(crystalSlot);
                        ItemStack current = inventory.getStack(0);
                        inventory.setStack(crystalSlot, current);
                        inventory.setStack(0, crystal);
                        crystalSlot = 0;
                    }
                    
                    // Переключаем на кристалл
                    ((org.stepan1411.pvp_bot.mixin.PlayerInventoryAccessor) inventory).setSelectedSlot(crystalSlot);
                }
                
                // Ставим кристалл на 5-м тике
                if (state.lastObsidianPos != null && state.crystalPvpTicks == 5) {
                    // Смотрим на верх обсидиана
                    Vec3d blockTop = new Vec3d(state.lastObsidianPos.getX() + 0.5, state.lastObsidianPos.getY() + 1.0, state.lastObsidianPos.getZ() + 0.5);
                    Vec3d botPos = bot.getEyePos();
                    double dx = blockTop.x - botPos.x;
                    double dy = blockTop.y - botPos.y;
                    double dz = blockTop.z - botPos.z;
                    double horizontalDist = Math.sqrt(dx * dx + dz * dz);
                    float yaw = (float) (Math.atan2(dz, dx) * (180.0 / Math.PI)) - 90.0f;
                    float pitch = (float) -(Math.atan2(dy, horizontalDist) * (180.0 / Math.PI));
                    bot.setYaw(yaw);
                    ...
                    try {
                        ...
                        state.crystalPvpStep = 2; // Переходим к шагу 3
                        state.crystalPvpTicks = 0;
                    } catch (Exception e) {
                        // Не удалось поставить - возвращаемся к шагу 0
                        state.crystalPvpStep = 0;
                        state.lastObsidianPos = null;
                        state.crystalPvpTicks = 0;
                    }
            ...
            case 2: // Шаг 3: Бьём кристалл
                // Переключаемся на оружие только в первый тик этого шага
                if (state.crystalPvpTicks == 1) {
                    int weaponSlot = findMeleeWeapon(inventory);
                    if (weaponSlot >= 0 && weaponSlot < 9) {
                        ((org.stepan1411.pvp_bot.mixin.PlayerInventoryAccessor) inventory).setSelectedSlot(weaponSlot);
                ...
                // Бьём кристалл на 5-м тике
                if (state.crystalPvpTicks == 5) {
                    Entity crystal = findNearestEndCrystal(bot, target, 5.0);
                    if (crystal != null) {
                        // Смотрим на кристалл
                        Vec3d crystalPos = crystal.getEyePos();
                        Vec3d botPos = bot.getEyePos();
                        double dx = crystalPos.x - botPos.x;
                        double dy = crystalPos.y - botPos.y;
                        double dz = crystalPos.z - botPos.z;
                        double horizontalDist = Math.sqrt(dx * dx + dz * dz);
                        float yaw = (float) (Math.atan2(dz, dx) * (180.0 / Math.PI)) - 90.0f;
                        float pitch = (float) -(Math.atan2(dy, horizontalDist) * (180.0 / Math.PI));
                        bot.setYaw(yaw);
                        ...
                        // Если враг рядом - спамим шаги 2 и 3
                        if (enemyNearby && state.lastObsidianPos != null) {
                            state.crystalPvpStep = 1; // Сразу ставим новый кристалл
                        } else {
                            state.crystalPvpStep = 0; // Начинаем цикл заново
                            state.lastObsidianPos = null;
                        }
                        state.crystalPvpTicks = 0;
                    } else {
                        // Кристалла нет - возвращаемся к шагу 1 или 0
                        if (state.lastObsidianPos != null) {
                            state.crystalPvpStep = 1; // Обсидиан есть - ставим кристалл
                        } else {
                            state.crystalPvpStep = 0; // Обсидиана нет - начинаем заново
                        }
                        state.crystalPvpTicks = 0;
                    }
                }
        ...
        // Держим дистанцию 3-5 блоков
        if (distance < 3.0) {
            BotNavigation.moveAway(bot, target, settings.getMoveSpeed() * 0.5);
        } else if (distance > 5.0) {
            BotNavigation.moveToward(bot, target, settings.getMoveSpeed() * 0.5);
        }
    }
    
    /**
     * Находит лучшую позицию для обсидиана - РЯДОМ с врагом (не под ним)
     * Проверяет блоки вокруг врага и выбирает ближайший свободный
     */
    private static net.minecraft.util.math.BlockPos findBestObsidianPosition(ServerPlayerEntity bot, Entity target, net.minecraft.world.World world) {
        net.minecraft.util.math.BlockPos targetPos = target.getBlockPos();
        
        // Проверяем блоки вокруг врага на уровне ног
        // Приоритет: ближайшие к врагу блоки
        net.minecraft.util.math.BlockPos[] candidates = {
            targetPos.north(),      // Север (z-1)
            targetPos.south(),      // Юг (z+1)
            targetPos.east(),       // Восток (x+1)
            targetPos.west(),       // Запад (x-1)
            targetPos.north().east(), // Северо-восток
            targetPos.north().west(), // Северо-запад
            targetPos.south().east(), // Юго-восток
            targetPos.south().west(), // Юго-запад
            targetPos.down(),       // Под ногами (если ничего не подошло)
        };
        ...
        for (net.minecraft.util.math.BlockPos pos : candidates) {
            ...
            // Проверяем что блок пустой или заменяемый И над ним есть место для кристалла
            if ((state.isAir() || state.isReplaceable()) && (above.isAir() || above.isReplaceable())) {
                // Проверяем что бот может достать до этой позиции (не дальше 5 блоков)
        ...
        // Если ничего не нашли - возвращаем позицию под ногами
        return targetPos.down();
    }
    
    /**
     * Ставит обсидиан на указанную позицию
     */
    private static boolean placeObsidian(ServerPlayerEntity bot, net.minecraft.util.math.BlockPos pos, net.minecraft.server.MinecraftServer server, net.minecraft.world.World world) {
        // Проверяем что блок пустой или заменяемый
        net.minecraft.block.BlockState currentState = world.getBlockState(pos);
        if (!currentState.isAir() && !currentState.isReplaceable()) {
            return false; // Блок занят
        }
        
        var inventory = bot.getInventory();
        int obsidianSlot = findObsidian(inventory);
        if (obsidianSlot < 0) return false;
    ...
    /**
     * Ставит End Crystal на обсидиан
     */
    private static boolean placeEndCrystal(ServerPlayerEntity bot, net.minecraft.util.math.BlockPos obsidianPos, net.minecraft.server.MinecraftServer server, net.minecraft.world.World world) {
        // Проверяем что на обсидиане можно поставить кристалл
        net.minecraft.util.math.BlockPos crystalPos = obsidianPos.up();
        net.minecraft.block.BlockState aboveState = world.getBlockState(crystalPos);
        if (!aboveState.isAir()) {
        ...
        // Смотрим на верх обсидиана
        Vec3d blockTop = new Vec3d(obsidianPos.getX() + 0.5, obsidianPos.getY() + 1.0, obsidianPos.getZ() + 0.5);
        Vec3d botPos = bot.getEyePos();
        double dx = blockTop.x - botPos.x;
        double dy = blockTop.y - botPos.y;
        double dz = blockTop.z - botPos.z;
     ...
     * Атакует End Crystal
     */
    private static void attackCrystal(ServerPlayerEntity bot, Entity crystal, net.minecraft.server.MinecraftServer server) {
        // Смотрим на кристалл
        Vec3d crystalPos = crystal.getEyePos();
        Vec3d botPos = bot.getEyePos();
        double dx = crystalPos.x - botPos.x;
        ...
        // Бьём кристалл через Carpet команду
        try {
            server.getCommandManager().getDispatcher().execute(
                "player " + bot.getName().getString() + " attack once", 
                server.getCommandSource()
    ...
    /**
     * Находит ближайший End Crystal рядом с целью
     */
    ...
    /**
     * Поиск обсидиана в инвентаре
     */
    private static int findObsidian(net.minecraft.entity.player.PlayerInventory inventory) {
        for (int i = 0; i < 36; i++) {
            ItemStack stack = inventory.getStack(i);
    ...
    /**
     * Поиск End Crystal в инвентаре
     */
    private static int findEndCrystal(net.minecraft.entity.player.PlayerInventory inventory) {
        for (int i = 0; i < 36; i++) {
            ItemStack stack = inventory.getStack(i);
    ...
    /**
     * Проверка наличия обсидиана
     */
    private static boolean hasObsidian(net.minecraft.entity.player.PlayerInventory inventory) {
        return findObsidian(inventory) >= 0;
    }
    
    /**
     * Проверка наличия End Crystal
     */
    private static boolean hasEndCrystal(net.minecraft.entity.player.PlayerInventory inventory) {
